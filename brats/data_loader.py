...
class DataLoader():
    ...
    def load_data(self):
        ...
        self.image_names = []
        self.in_size= []
        self.bbox   = h5py.File(os.path.join('hdf5','bbox.h5'), 'a')
        self.data   = h5py.File(os.path.join('hdf5','data.h5'), 'a')
        self.weight = h5py.File(os.path.join('hdf5','weight.h5'), 'a')
        self.label  = h5py.File(os.path.join('hdf5','label.h5'), 'a')
        ...
        for i in range(data_num):
            ...
            for mod_idx in range(len(self.modality_postfix)):
                ...
                if(mod_idx ==0):
                    a_weight = np.asarray(volume > 0, np.float32)
            ...
            self.image_names.append(volume_name_list)
            self.data.create_dataset(str(i), data=np.asarray(volume_list))
            self.weight.create_dataset(str(i), data=a_weight)
            self.bbox.create_dataset(str(i), data=np.array([bbmin, bbmax]))
            self.in_size.append(volume_size)
            ...
            if(self.with_ground_truth):
                ...
                self.label.create_dataset(str(i), data=label)
    ...
    def __get_one_batch(self):
        ...
        for i in range(batch_size):
            if(self.with_flip):
                ...
                flip = False
            self.patient_id = random.randint(0, len(list(self.data.keys()))-1)
            data_volumes = [x for x in np.asarray(self.data[str(self.patient_id)].value)]
            weight_volumes = [np.asarray(self.weight[str(self.patient_id)].value)]
            boundingbox = None
            if(self.with_ground_truth):
                label_volumes = [np.asarray(self.label[str(self.patient_id)].value)]
                if(train_with_roi_patch):
    ...
    def get_image_data_with_name(self, i):
        ...
        """
        i = str(i)
        return [self.data[i], self.weight[i], self.patient_names[i], self.image_names[int(i)], self.bbox[i], self.in_size[int(i)]]
